document.addEventListener('DOMContentLoaded', async () => {
    ...
    function initializeDOMElements() {
        ...
        try { // Adiciona try-catch para facilitar debug de elementos não encontrados
            ...
            const essentialElements = [
                studioView, loginPrompt, playerSelect, newSingleForm, newAlbumForm, featModal,
                singleReleaseDateInput, albumReleaseDateInput, trackTypeModal, albumTrackModal,
                openAddTrackModalBtn, inlineFeatAdder, inlineFeatArtistSelect, confirmInlineFeatBtn, addInlineFeatBtn
            ];
            if (!allViews || allViews.length === 0 || essentialElements.some(el => !el)) {
                 console.error("ERRO CRÍTICO: Elementos essenciais do HTML não foram encontrados!", {
                    missing: essentialElements.map((el, i) => el ? null : i).filter(x => x !== null)
                 });
    ...
    const setupCountdown = (timerId, callback) => { /* ... código inalterado ... */
    ...
    const calculateSimulatedStreams = (points, lastActiveISO) => { /* ... código inalterado ... */
    ...
    const computeChartData = (artistsArray) => { /* ... código inalterado ... */
    ...
    function renderRPGChart() { /* ... código inalterado ... */
        const chartData = computeChartData(db.artists); const container = document.getElementById('artistsGrid'); if (!container) { console.error("Container 'artistsGrid' não encontrado."); return; } if (chartData.length === 0) { container.innerHTML = '<p class="empty-state">Nenhum artista no chart de RPG.</p>'; return; } container.innerHTML = chartData.map((artist, index) => `<div class="artist-card" data-artist-name="${artist.name}"> <span class="rpg-rank">#${index + 1}</span> <img src="${artist.img}" alt="${artist.name}" class="artist-card-img"> <p class="artist-card-name">${artist.name}</p> <span class="artist-card-type">${(artist.streams || 0).toLocaleString('pt-BR')} streams</span> </div>`).join('');
     }
