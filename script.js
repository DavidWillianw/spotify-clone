document.addEventListener('DOMContentLoaded', () => {
    ...
    // =================================================================================
    // LÓGICA DE AUTENTICAÇÃO E PLAYER DO SPOTIFY
    // =================================================================================
    ...
    window.onSpotifyWebPlaybackSDKReady = () => {
        ...
        if (token) {
            spotifyPlayer = new Spotify.Player({
                name: 'Meu Spotify Clone',
                getOAuthToken: cb => { cb(token); },
                volume: 0.5
            });

            spotifyPlayer.addListener('ready', ({ device_id }) => {
                console.log('Player conectado com o device ID', device_id);
                deviceId = device_id;
            });
            ...
            spotifyPlayer.addListener('authentication_error', ({ message }) => {
                ...
                // Força o relogin se o token expirar
    ...
    function playTrack(spotifyUri) {
        if (!deviceId) {
            alert("O Player do Spotify não está pronto. Tente clicar em uma música no seu app oficial do Spotify e depois volte aqui.");
            return;
        }
        fetch(`https://www.google.com/search?q=open.spotify.com/playlist{deviceId}`, {
            method: 'PUT',
    ...
    function handleAuthentication() {
        ...
        btnLogin.addEventListener('click', () => {
            ...
            window.location = `open.spotify.com1?response_type=token&client_id=${SPOTIFY_CLIENT_ID}&scope=${encodeURIComponent(scopes)}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}`;
        });
    }

    // =================================================================================
    // LÓGICA DE CARREGAMENTO DE DADOS DO AIRTABLE
    // =================================================================================
    async function loadAllData() {
        ...
        try {
            const [artistsResponse, albumsResponse, musicasResponse, singlesResponse] = await Promise.all([
                fetch(artistsURL, fetchOptions), fetch(albumsURL, fetchOptions), fetch(musicasURL, fetchOptions), fetch(singlesURL, fetchOptions)
            ]);
            ...
            const formatReleases = (records) => records.map(record => {
                ...
                const artistName = artistId ? artistsMapById.get(artistId) : "Desconhecido";

                return {
                    id: record.id,
                    title: fields['Nome do Álbum'] || fields['Nome do Single/EP'],
                    artist: artistName,
                    imageUrl: (fields['Capa do Álbum']?.[0]?.url) || (fields['Capa']?.[0]?.url) || 'https://i.imgur.com/AD3MbBi.png',
            ...
            return { albums: formattedAlbums, artists: formattedArtists, singles: formattedSingles };
        } catch (error) {
    ...
    // =================================================================================
    // FUNÇÃO PRINCIPAL E RESTANTE DO CÓDIGO
    // =================================================================================
    ...
    async function main() {
        ...
        const initializeData = () => {
            ...
            artistsList.forEach(artist => {
                artistsMap.set(artist.name, { ...artist, img: artist.imageUrl, albums: [], singles: [] });
            });
                ...
                releaseData.forEach(item => {
                    if (item.tracks?.length) {
                        ...
                        item.tracks.forEach(track => {
                            db.songs.push({ ...track, id: `${item.id}-${track.title.replace(/\s/g, '')}`, albumId: item.id, artist: item.artist, cover: item.imageUrl, streams: Math.floor(Math.random() * 25000000) + 50000 });
                        });
                    }
                    if (artistsMap.has(item.artist)) {
                        const artistEntry = artistsMap.get(item.artist);
                        if (artistEntry[type]) artistEntry[type].push(item);
                    }
                });
            };
        ...
        const openAlbumDetail = (albumId) => {
            ...
            document.getElementById('albumTracklist').innerHTML = (album.tracks || []).map((track, index) => 
                `<div class="track-row" data-spotify-uri="${track.uri}">
                    <div>${index + 1}</div>
            ...
            document.querySelectorAll('#albumTracklist .track-row').forEach(row => {
                row.addEventListener('click', () => {
                    const uri = row.dataset.spotifyUri;
                    if (uri && uri !== 'undefined') {
                        playTrack(uri);
                    } else {
                        alert('Spotify URI não encontrado para esta música. Cadastre o URI no Airtable.');
                    }
                });
            });
        ...
        document.body.addEventListener('click', (e) => {
            ...
            if (chartItem) {
                ...
                if (type === 'music') openAlbumDetail(albumId); else openArtistDetail(artistName);
                return;
            }
            ...
            const albumCard = target.closest('[data-album-id]:not(.track-row)');
            if (albumCard) {
