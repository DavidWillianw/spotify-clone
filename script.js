document.addEventListener("DOMContentLoaded", async () => {
  async function loadData() {
    try {
      ...
      console.error("Falha ao carregar os dados do data.json:", error);
      return [];
    }
  }

  const albumsData = await loadData();
  ...
  const initializeData = () => {
    ...
    // MUDANÇA AQUI: Agora lemos as tracks do JSON e calculamos a duração
    albumsData.forEach((album) => {
      ...
      if (album.tracks && album.tracks.length > 0) {
        ...
        // Adiciona as tracks do álbum ao DB de músicas global
        album.tracks.forEach((track) => {
          db.songs.push({
            ...track,
            id: `${album.id}-${track.title.replace(/\s/g, "")}`,
            albumId: album.id,
            artist: album.artist,
  ...
  const handleBack = () => {
    if (viewHistory.length > 1) {
      ...
      allViews.forEach((v) =>
        v.classList.toggle("hidden", v.id !== previousViewId)
      );
      window.scrollTo(0, 0);
    }
  };
  ...
  document
    ...
    .forEach((btn) => btn.addEventListener("click", handleBack));
  ...
  const renderArtistsGrid = (containerId, artists) => {
    ...
    container.innerHTML = artists
      .map(
        (artist) =>
          `<div class="artist-card" data-artist-name="${artist.name}"><img src="${artist.img}" alt="${artist.name}"><h3>${artist.name}</h3></div>`
      )
      .join("");
  };
  ...
  const renderChart = (type) => {
    ...
    const items =
      type === "music"
        ...
        : [...db.albums].sort((a, b) => b.metascore - a.metascore).slice(0, 50);
    container.innerHTML = items
      .map((item, index) => {
        const trends = ["up", "down", "new", "same"];
        const trend = trends[Math.floor(Math.random() * trends.length)];
        let trendIcon = "";
        if (trend === "up")
          trendIcon = `<i class="fas fa-caret-up trend-up"></i>`;
        else if (trend === "down")
          trendIcon = `<i class="fas fa-caret-down trend-down"></i>`;
        else if (trend === "new")
          trendIcon = `<span class="trend-new">NEW</span>`;
        else trendIcon = `<span>-</span>`;
        return `<div class="chart-item" data-id="${
          item.id
        }" data-type="${type}" data-artist-name="${
          item.artist
        }" data-album-id="${
          item.albumId || item.id
        }"><div class="chart-position">${index + 1}</div><img src="${
          item.cover || item.imageUrl
        }" class="chart-cover"><div class="chart-info"><div class="chart-title">${
          item.title
        }</div><div class="chart-artist">${
          item.artist
        }</div></div><div class="chart-stats"><div class="chart-streams">${(
          item.streams || item.metascore * 10000
        ).toLocaleString(
          "pt-BR"
        )}</div><div class="chart-trend">${trendIcon}</div></div></div>`;
      })
      .join("");
  };
  ...
  const openArtistDetail = (artistName) => {
    ...
    document.getElementById("popularSongsList").innerHTML = topSongs
      .map(
        (song, index) =>
          `<div class="song-row" data-album-id="${
            song.albumId
          }"><div style="color: var(--text-secondary);">${
            index + 1
          }</div><div class="song-row-info"><img class="song-row-cover" src="${
            song.cover
          }" alt="${song.title}"><div class="song-row-title">${
            song.title
          }</div></div><div class="song-streams">${song.streams.toLocaleString(
            "pt-BR"
          )}</div></div>`
      )
      .join("");
    ...
    const renderHorizontalList = (containerId, items) => {
      document.getElementById(containerId).innerHTML = items
        .map(
          (item) =>
            `<div class="album-card" data-album-id="${item.id}"><img src="${
              item.imageUrl
            }" alt="${item.title}"><div class="album-title">${
              item.title
            }</div><div class="album-year">${new Date(
  ...
  const openAlbumDetail = (albumId) => {
    ...
    const totalMinutes = Math.floor(album.totalDurationSeconds / 60);
    document.getElementById(
      "albumDetailInfo"
    ).innerHTML = `<strong class="clickable-artist" data-artist-name="${
      album.artist
    }">${album.artist}</strong> • ${new Date(
      album.releaseDate || "2024-01-01"
    ).getFullYear()} • ${album.tracks.length} músicas, ${totalMinutes} min`;
    document.getElementById("albumTracklist").innerHTML = (album.tracks || [])
      .map(
        (track, index) =>
          `<div class="track-row"><div>${
            index + 1
          }</div><div class="track-title">${
            track.title
          }</div><div class="track-duration">${track.duration}</div></div>`
      )
      .join("");
  ...
  const openDiscographyDetail = (type) => {
    ...
    const items =
      type === "albums" ? activeArtist.albums : activeArtist.singles;
    document.getElementById("discographyTypeTitle").textContent =
      type === "albums" ? "Todos os Álbuns" : "Todos os Singles";
    const grid = document.getElementById("discographyGrid");

    // MUDANÇA AQUI: Usa a classe correta para renderizar os cards de álbum
  ...
  const handleSearch = () => {
    ...
    if (query) {
      const filtered = db.artists.filter((a) =>
  ...
  const switchTab = (event, forceTabId = null) => {
    const tabId = forceTabId || event.currentTarget.dataset.tab;
  ...
  const setupCountdown = (timerId, callback) => {
    ...
    const updateTimer = () => {
      ...
      if (duration === 59) callback();
      timerDisplay.textContent = `00:${String(duration).padStart(2, "0")}`;
    };
    updateTimer();
  ...
  renderArtistsGrid(
    "homeGrid",
    [...db.artists].sort(() => 0.5 - Math.random()).slice(0, 10)
  );
