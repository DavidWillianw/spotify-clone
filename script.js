document.addEventListener('DOMContentLoaded', async () => {
    ...
    let allViews, searchInput, studioView, loginPrompt, loggedInInfo, playerSelect,
        ...
        addTrackButton, albumTracklistEditor,
    ...
    function initializeDOMElements() {
        ...
        try {
            ...
            addTrackButton = document.getElementById('addTrackButton'); // Removido do form album
    ...
    // ATUALIZADO: renderChart inclui indicador
    ...
    // ATUALIZADO: renderRPGChart com indicadores
    function renderRPGChart() { /* ... código inalterado ... */
    ...
    // --- 4. SISTEMA DO ESTÚDIO --- (Sem mudanças internas nas funções, apenas ajuste no listener do add-feat-btn)
    ...
    async function main() {
        ...
        if (data && data.allArtists) {
            if (!initializeData(data)) return; // Carrega dados do Airtable E do localStorage

            try {
                initializeStudio(); // Configura listeners, etc.
                ...
                // Salva os charts iniciais no localStorage para a *próxima* vez que carregar
                saveChartDataToLocalStorage('music');
                saveChartDataToLocalStorage('album');
                saveChartDataToLocalStorage('rpg');

                // Renderiza os charts usando os dados carregados (que podem ser vazios na primeira vez)
                renderRPGChart();
                renderChart('music');
                renderChart('album');
                ...
                // Configura os contadores (eles chamarão save E render quando zerarem)
                setupCountdown('musicCountdownTimer', 'music');
                setupCountdown('albumCountdownTimer', 'album');
                // setupCountdown('rpgCountdownTimer', 'rpg'); // Se precisar
